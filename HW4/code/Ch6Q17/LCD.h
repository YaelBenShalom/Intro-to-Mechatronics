...
void LCD_WriteString(
    const char *string);       // Write string starting at current position
...
void LCD_Display(int d, int c,
                 int b);         // Turn display on/off and set cursor settings
...
void LCD_CustomChar(unsigned char val,
                    const char data[7]);    // Write custom char to CGRAM
