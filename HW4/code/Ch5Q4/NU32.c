...
// These only have an effect for standalone programs but don't harm bootloaded
// programs. the settings here are the same as those used by the bootloader
...
#pragma config UPLLIDIV =                                                      \
    DIV_2                    // Divide 8 MHz input by 2, mult by 12 for 48 MHz
#pragma config FUSBIDIO = ON // USBID controlled by USB peripheral when it is on
#pragma config FVBUSONIO =                                                     \
    ON // VBUSON controlled by USB peripheral when it is on
...
#pragma config FSRSSEL =                                                       \
    PRIORITY_6 // Shadow Register Set for interrupt priority 6
