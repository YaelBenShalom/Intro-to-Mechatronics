...
void LCD_Setup() {
  int en = __builtin_disable_interrupts(); // disable interrupts, remember
                                           // initial state
  ...
  // account for variations in timing amongst different HD44780 compatible
  // parts. The timing we use here is for the KS066U which is faster than the
  // HD44780.
  PMMODEbits.WAITB = 0x3; // Tas in the LCD datasheet is 60 ns
  PMMODEbits.WAITM =
      0xF; // PWeh in the data sheet is 230 ns (we don't quite meet this)
  ...
  LCD_Display(
      1, 0, 0); // Display control: display on, cursor off, blinking cursor off
...
void LCD_Home(void) { LCD_Write(0, 0x02); }
...
void LCD_Entry(int id, int s) { LCD_Write(0, 0x04 | (id << 1) | s); }
...
void LCD_Shift(int sc, int rl) { LCD_Write(0, 0x1 | (sc << 3) | (rl << 2)); }
...
void LCD_Function(int n, int f) { LCD_Write(0, 0x30 | (n << 3) | (f << 2)); }
...
void LCD_Move(int line, int col) { LCD_Write(0, 0x80 | (line << 6) | col); }

// Sets the CGRAM address, used for creating custom
// characters
// addr address in the CGRAM to make current
void LCD_CMove(unsigned char addr) { LCD_Write(0, 0x40 | addr); }

// Writes the character to the LCD at the current position
void LCD_WriteChar(char c) { LCD_Write(1, c); }
...
unsigned char LCD_Read(int rs) {
  volatile unsigned char val =
      0;    // volatile so 1st read doesn't get optimized away
  if (rs) { // 1 to read data
    PMADDRSET = 1 << PMABIT;
  } else { // 0 to read command status
    PMADDRCLR = 1 << PMABIT;
  }
  // from the PIC32 reference manual, you must read twice to actually get the
  // data
  waitPMP(); // wait for the PMP to be ready
  val = PMDIN;
  waitPMP();
