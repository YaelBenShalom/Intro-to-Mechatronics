...
void __ISR(_TIMER_2_VECTOR, IPL3SOFT) Controller(void) { // _TIMER_2_VECTOR = 8
  ...
  case ITEST: {
    ...
    OC1RS = curr_PI(kp, ki, curr_ref_test[curr_Count],
                    curr_actual_test[curr_Count]);
    set_direction(dir);
    ...
    //   OC1RS = position_PID(pos_kp, pos_ki, pos_kd, pos_ref_test[pos_Count],
    //   pos_actual_test[pos_Count]); set_direction(dir);
...
void __ISR(_TIMER_4_VECTOR, IPL5SOFT) positioncontrol(void) { // Timer 4
  ...
  if (MODE == TRACK) {
    ...
    curr_ref = position_PID(pos_kp, pos_ki, pos_kd, traj_ref[traj_i],
                            traj_actual[traj_i]);
